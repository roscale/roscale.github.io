...
Once a jthread is created, a getter lets you retrieve the stop source.
This object can passed to other threads without any problem because it's implemented as an atomic boolean under the hood.
...
And the third one related to the second is not being able to return a value from a thread.
...
My guess is that official implementation would have to synergise with the existing cancellation API, which is hard to implement.
...
The thread can check the channel status by trying to dequeue from it in a non-blocking fashion.
It can also wait on the channel if you want to send items alongside the cancel notification.
A disadvantage of implementing cancellation in this way is that you need to be able to manually drop the sender at will.
This probably means wrapping the sender in an `Option` so you can replace the value with `None`.
And if you want to cancel from any thread, you need to make it thread-safe.
One solution is `Arc<Refcell<Option<mpsc::Sender<T>>>>`.
...
The thread would need to handle both stop and channel closed.
And you still have to solve the the implementation leaking.
No one should care about the entire channel type if all you want is to send a stop signal.
C++'s stop signal is perfect because it's copiable, thread safe, and it's only purpose is to request to stop.

# Conclusion

C++ implements cancellation while ommitting message passing.
Rust does the opposite.

I don't think it's impossible to have channels and cancellation synergize with each other, so I made [this repo](https://github.com/roscale/cancellation_rs) to try to implement them from scratch to achieve this goal.
